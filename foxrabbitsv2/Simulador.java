...
public class Simulador
{
    ...
    // The probability that a lion may be created in any given position
    ...
    public Simulador(int depth, int width)
    {
        ...
        view.setColor(Leao.class, Color.CYAN);
    ...
    public void simulate(int numSteps)
    {
        for(int step=1; step <= numSteps && view.isViable(field); step++) {
            simulateOneStep();
             delay(400);   // uncomment this to run more slowly
        }
    }
    ...
    public void simulateOneStep()
    {
      ...
      //  for(Coelho rabbit: rabbits) {
        ...
        // Add the newly born raposas ,rabbits and lions to the main lists.
        ...
        for(int row = 0; row < field.getDepth(); row++) {
            for(int col = 0; col < field.getWidth(); col++) {
                if(rand.nextDouble() <= FOX_CREATION_PROBABILITY) {
                    ...
                    //acrescentar o else if para o LEAO para criar o Leao
                } else if (rand.nextDouble() <= LION_CREATION_PROBABILITY ) {
                    Localizacao location = new Localizacao(row,col);
                    Leao lion = new Leao(true,field,location);
                    animals.add(lion);
